...
describe('`<CheckboxFilter /> header`', () => {
    ...
    it('fires `onChange` with all values when clicked and unchecked', () => {
        ...
        const emptyWrapper = shallow(<CheckboxFilter
            title="Field Name"
            value={[]}
            filterValues={filterValues}
            onChange={emptyOnChange}
        />)
        emptyWrapper.find(Checkbox).at(0).simulate('change', { target: { checked: true } })
        expect(emptyOnChange).toBeCalledWith(expect.arrayContaining(['one', 'two', 'three']))
        const partialOnChange = jest.fn()
        const partialWrapper = shallow(<CheckboxFilter
            title="Field Name"
            value={['one', 'three']}
            filterValues={filterValues}
            onChange={partialOnChange}
        />)
        partialWrapper.find(Checkbox).at(0).simulate('change', { target: { checked: true } })
    ...
    it('fires `onChange` with null when clicked and checked', () => {
        ...
        const wrapper = shallow(<CheckboxFilter
            ...
            onChange={onChange}
        />)
        wrapper.find(Checkbox).at(0).simulate('change', { target: { checked: false } })
...
describe('`<CheckboxFilter /> list`', () => {
    it('renders `Checkbox` and title for each item', () => {
        ...
        expect(wrapper.find('ul').find('li').map(item => item).every((item, index) => {
    ...
    it('fires `onChange` with checked value', () => {
        ...
        const wrapper = shallow(<CheckboxFilter
            ...
            onChange={onChange}
        />)
        wrapper.find('ul').find(Checkbox).at(0).simulate('change', { target: { value: filterValues[0].value, checked: true } })
    ...
    it('fires `onChange` without checked value', () => {
        ...
        const wrapper = shallow(<CheckboxFilter
            ...
            onChange={onChange}
        />)
        wrapper.find('ul').find(Checkbox).at(0).simulate('change', { target: { value: filterValues[2].value, checked: false } })
        ...
        wrapper.find('ul').find(Checkbox).at(0).simulate('change', { target: { value: filterValues[1].value, checked: false } })
        expect(onChange).toBeCalledWith(null)
    })
    ...
    it('handles empty value', () => {
        ...
        const wrapper = shallow(<CheckboxFilter
            ...
            onChange={onChange}
        />)
        wrapper.find('ul').find(Checkbox).at(0).simulate('change', { target: { value: filterValues[2].value, checked: true } })
