...
export const apiError: Epic = (action$, store) => action$.ofType(types.apiError)
.mergeMap((action) => {
    return apiErrorImpl(action, store)
})

export function apiErrorImpl(
    action: ActionsMap['apiError'],
    store: Store<CoreStore, AnyAction>
): Observable<AnyAction> {
    const { error, callContext } = action.payload
    if (error.response) {
        return Observable.of($do.httpError({
            statusCode: error.response.status,
            error,
            callContext
        }))
    } else if (!axios.isCancel(error)) {
        return Observable.of($do.showViewError({
            error: {
