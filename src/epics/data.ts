        ...
        .mergeMap(action => {
            ...
            if (action.type === types.bcForceUpdate) {
                const infinityPaginationWidget =
                    (widgetName && state.view.infiniteWidgets.includes(widgetName)) ||
                    state.view.widgets?.filter(item => item.bcName === bcName)?.find(item => state.view.infiniteWidgets.includes(item.name))
                        ?.name
