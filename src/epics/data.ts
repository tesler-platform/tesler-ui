        ...
        mergeMap(action => {
            ...
            const normalFlow = api.fetchBcData(state.screen.screenName, bcUrl, fetchParams, canceler.cancelToken).pipe(
                ...
                catchError((error: any) => {
                    console.error(error)
                    return observableOf($do.bcFetchDataFail({ bcName, bcUrl }))
                })
            )

            return observableRace(cancelFlow, normalFlow)
        })
    )
    ...
    action$.pipe(
        ...
        mergeMap(action => {
            ...
            return observableConcat(
                observableOf($do.bcChangeCursors({ cursorsMap: { [bcName]: cursor }, keepDelta: action.payload.keepDelta })),
...
export const inlinePickListFetchDataEpic: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.inlinePickListFetchDataRequest),
        mergeMap(action => {
            ...
            const normalFlow = api
                .fetchBcData(store$.value.screen.screenName, bcUrl, { [searchSpec + '.contains']: searchString }, canceler.cancelToken)
                .pipe(
                    mergeMap(data => {
                        return observableOf($do.bcFetchDataSuccess({ bcName, data: data.data, bcUrl }))
                    }),
                    catchError((error: any) => {
                        console.error(error)
        ...
        mergeMap(action => {
            ...
            return api.deleteBcData(state.screen.screenName, bcUrl, context).pipe(
                mergeMap(data => {
                    ...
                    return observableConcat(
                        isTargetFormatPVF ? observableOf($do.bcCancelPendingChanges({ bcNames: [bcName] })) : EMPTY,
                        observableOf($do.bcFetchDataRequest({ bcName, widgetName })),
                        postInvoke ? observableOf($do.processPostInvoke({ bcName, postInvoke, cursor, widgetName })) : EMPTY
                    )
                }),
                catchError((error: any) => {
                    console.log(error)
...
export const saveAssociationsPassive: Epic = (action$, store$) =>
    action$.pipe(
        ...
        switchMap(action => {
            ...
            action.payload.bcNames.forEach((pendingBc: string) => {
...
export const changeChildrenAssociations: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.changeChildrenAssociations),
        mergeMap(action => {
            const state = store$.value
...
export const changeChildrenAssociationsSameBc: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.changeChildrenAssociationsSameBc),
        mergeMap(action => {
            const state = store$.value
...
export const changeDescendantsAssociationsFull: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.changeDescendantsAssociationsFull),
        mergeMap(action => {
            const state = store$.value
...
export const changeAssociation: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.changeAssociation),
        mergeMap(action => {
            const state = store$.value
...
export const changeAssociationSameBc: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.changeAssociationSameBc),
        mergeMap(action => {
            const bcName = action.payload.bcName
            const result: Array<Observable<AnyAction>> = [
                observableOf(
...
export const changeAssociationFull: Epic = (action$, store$) =>
    action$.pipe(
        ofType(types.changeAssociationFull),
        mergeMap(action => {
            const state = store$.value
            const result: Array<Observable<AnyAction>> = []
