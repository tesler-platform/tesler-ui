...
describe('combineMiddlewares test', () => {
    const testAutosave = (store: MiddlewareAPI<Dispatch<AnyAction>, CoreStore>) =>
        (next: Dispatch) =>
            (action: AnyAction) => {
                return next(action)
            }
    ...
    it('should return core middlewares', () => {
        ...
        expect(
            combineMiddlewares(coreMiddlewares).findIndex(i => i.name === coreMiddlewares.autosave.name)
        ).toEqual(coreMiddlewaresArray.findIndex(i => i.name === coreMiddlewares.autosave.name))
    })
    it('should disable `autosave`', () => {
        ...
        expect(
            callResult.findIndex(i => i.name === coreMiddlewares.autosave.name)
        ).toEqual(-1)
        expect(
            callResult[callResult.length - 1].name
        ).toEqual(coreMiddlewaresArray[coreMiddlewaresLength - 1].name)
    ...
    it('should replace core middleware', () => {
        ...
        expect(
            callResult.findIndex(i => i.name === testCustomMiddewares.autosave.name)
        ).toEqual(coreMiddlewaresArray.findIndex(i => i.name === coreMiddlewares.autosave.name))
    })
})
